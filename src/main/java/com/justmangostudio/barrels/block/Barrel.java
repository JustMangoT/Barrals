package com.justmangostudio.barrels.block;
...
import com.justmangostudio.barrels.*;
...
public abstract class Barrel extends SimpleSlimefunItem<BlockTicker> {
    ...
    private ItemStack getCapacityItem(Block b) {
        ...
        if (percentage < 25) bar.append("&2");
        else if (percentage < 50) bar.append("&a");
    ...
    private void updateBarrel(Block b) {
        ...
        if (inventory.getItemInSlot(getOutputSlots()[0]) != null) {
            if (isSimilar(inventory.getItemInSlot(getOutputSlots()[0]), output))
                output.setAmount(Math.min(stored, output.getMaxStackSize() - inventory.getItemInSlot(getOutputSlots()[0]).getAmount()));
        } else output.setAmount(Math.min(stored, output.getMaxStackSize()));
